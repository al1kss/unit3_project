...
class OrderItemRow(MDCard):
    ...
    def on_order_data(self, instance, value): #dix later
        if len(value) < 7:
            defaults = ["Unknown", "Unknown", "Unknown", "Unknown", 0, "Unknown", "Awaiting"]
...
class AdminOrdersScreen(Screen):
    ...
    def update_order_status(self, order_id, new_status):
        #updateorders.sql and reload the orders list
        db = DatabaseManager(name="orders.sql")
...
class CartScreen(Screen):
    ...
    def proceed_to_checkout(self):
        ...
        #if lgged in
        ticket_num = random.randint(10, 99)  
        ...
        for item in app.cart:
            ...
            official_price = row[0] if row else item["price"]  
